...
class Mixer(torch.nn.Module):
  ...
  def __init__(self, config, n_repeat):
    super().__init__()
    self.n_repeat = n_repeat
    eps = 1e-4
    self.weight = nn.ParameterList([nn.Parameter(torch.ones((1,))* (1.0 if i == (self.n_repeat-1) else eps)) for i in range(self.n_repeat)])
    
  def forward(self, x_acc):
    B, T, D = x_acc[0].shape
    weighted_avg = x_acc[0] * self.weight[0]
    for i in range(1, len(self.weight)):
        weighted_avg = weighted_avg + self.weight[i] *  x_acc[i]
    return weighted_avg
...
class GPTBase(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.mixer = Mixer(config, n_repeat=self.n_repeat+1)
    ...
    def forward(self, idx, targets=None, get_logits=False, use_cache=False, iter=None):
        ...
        B, T, D = x.shape
        base_indices = torch.arange(T, device=x.device) #index_shift + torch.arange(T, device=x.device) * self.n_repeat
        indices = base_indices
 
        x_acc = [x]

        for rep_idx in range(1, self.n_repeat+1):
            x = pos_emb_closure.adapt_model_input(x, indices=indices)
            x = self.transformer.drop(x)

            for block in self.transformer.h[rep_idx-1]:
                x = block(x, pos_emb_closure, cache_context, start_index=None, indices=indices)
            
            x_acc.append(x) # torch.cat([x_in.view(B,T,-1), x], dim=-1).view(B,-1,D) # interleaves the output with the previous input, now the sequence length grows by T
    ...
    def get_parameter_group_specs(self):
        ...
        decay = set()
        no_decay = set()
        whitelist_weight_modules = (torch.nn.Linear, Mixer)
