...
class Block(nn.Module):
    ...
    def __init__(self, config, lm_cache):
        ...
        self.mlp = MLP(config)
        self.gain_attn = nn.Parameter(torch.ones((1,)))
        self.gain_mlp = nn.Parameter(torch.ones((1,)))

    def forward(self, x, pos_emb_closure, cache_context, start_index):
        x = self.gain_attn * x + self.attn(self.ln_1(x), pos_emb_closure, cache_context, start_index)
        x = self.gain_mlp * x + self.mlp(self.ln_2(x))
        return x
